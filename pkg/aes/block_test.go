...
func TestKeyExpansion(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			nk, nr := getNkAndNr(tc.key)
			actual := keyExpansion(tc.key, nk, nr)
...
func TestCipher(t *testing.T) {
	for _, tc := range cipherCases {
		t.Run(tc.name, func(t *testing.T) {
			actual := Cipher(tc.in, tc.key)
			if !bytes.Equal(actual, tc.out) {
				t.Errorf("expected %x, got %x", tc.out, actual)
			}
		})
	}
}
...
func TestInvCipher(t *testing.T) {
	for _, tc := range cipherCases {
		t.Run(tc.name, func(t *testing.T) {
			actual := InvCipher(tc.out, tc.key)
			if !bytes.Equal(actual, tc.in) {
				t.Errorf("expected %x, got %x", tc.in, actual)
			}
		})
	}
}
