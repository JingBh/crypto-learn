...
// https://cs.opensource.google/go/go/+/refs/tags/go1.22.2:src/crypto/des/des_test.go
var encryptDESTests = []struct {
	key []byte
	in  []byte
	out []byte
}{{
	...
	[]byte{0x73, 0x65, 0x63, 0x52, 0x33, 0x74, 0x24, 0x3b},
	[]byte{0x61, 0x20, 0x74, 0x65, 0x73, 0x74, 0x31, 0x32},
	...
	[]byte{0x1f, 0x79, 0x90, 0x5f, 0x88, 0x01, 0xc8, 0x88},
	[]byte{0xc7, 0x46, 0x18, 0x73, 0xaf, 0x48, 0x5f, 0xb3},
	[]byte{0xb0, 0x93, 0x50, 0x88, 0xf9, 0x92, 0x44, 0x6a},
}, {
	[]byte{0xe6, 0xf4, 0xf2, 0xdb, 0x31, 0x42, 0x53, 0x01},
	[]byte{0xff, 0x3d, 0x25, 0x50, 0x12, 0xe3, 0x4a, 0xc5},
	[]byte{0x86, 0x08, 0xd3, 0xd1, 0x6c, 0x2f, 0xd2, 0x55},
}, {
	[]byte{0x69, 0xc1, 0x9d, 0xc1, 0x15, 0xc5, 0xfb, 0x2b},
	[]byte{0x1a, 0x22, 0x5c, 0xaf, 0x1f, 0x1d, 0xa3, 0xf9},
	[]byte{0x64, 0xba, 0x31, 0x67, 0x56, 0x91, 0x1e, 0xa7},
}, {
	[]byte{0x6e, 0x5e, 0xe2, 0x47, 0xc4, 0xbf, 0xf6, 0x51},
	[]byte{0x11, 0xc9, 0x57, 0xff, 0x66, 0x89, 0x0e, 0xf0},
	[]byte{0x94, 0xc5, 0x35, 0xb2, 0xc5, 0x8b, 0x39, 0x72},
}}

func TestPermute(t *testing.T) {
	var table = []byte{2, 4, 5, 7, 8, 3, 6, 1}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := permute([]byte{tc.input}, table)[0]
...
func TestEncipherBlock(t *testing.T) {
	for i, tt := range encryptDESTests {
		out := EncipherBlock(tt.in, tt.key)
...
func TestDecipherBlock(t *testing.T) {
	for i, tt := range encryptDESTests {
		out := DecipherBlock(tt.out, tt.key)
